...
class Dataset:
    def __init__(self, filenames, input_size, params, augment, dtype=tf.float32):
        self.params = params
        self.mosaic = augment
    ...
    def __getitem__(self, index):
        ...
        # Convert BGR to RGB and keep as HWC for Keras (channels last)
        sample = image[..., ::-1].astype(np.float32) / 255.0  # Normalize to [0,1]
        sample = np.ascontiguousarray(sample)
        

        if not mosaic:
            # Convert to tensor format for Keras: [[orig_h, orig_w], [ratio, ratio], [pad_w, pad_h]]
            # ratio is a tuple (r, r) so we use the padding values instead
            shapes = np.array([[orig_h, orig_w], [ratio[0], ratio[1]], [pad_w, pad_h]], dtype=np.float32)
        else:
            # Use zeros for mosaic samples - maintain (3, 2) format
            shapes = np.zeros((3, 2), dtype=np.float32)

        # Convert to the specified dtype
        sample_dtype = tf.convert_to_tensor(sample, dtype=self.dtype)
        target_dtype = tf.convert_to_tensor(target, dtype=self.dtype)
        
        return sample_dtype, target_dtype, shapes
    ...
    def load_label(self, filenames):
        cache_path = os.path.join(os.path.dirname(filenames[0]), 'labels.cache')
        if os.path.exists(cache_path):
            try:
                # Load cache file using numpy
                cache_data = np.load(cache_path, allow_pickle=True)
                return cache_data.item()  # Convert from numpy array to dictionary
